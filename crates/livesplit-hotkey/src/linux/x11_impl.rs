                        ...
                        for message in receiver.try_iter() {
                            match message {
                                ...
                                Message::Resolve(key_code, promise) => {
                                    promise.set(resolve(&xlib, display, key_code))
                                }
                                Message::End => {
                                    break 'event_loop;
                                }
...
pub(super) fn resolve(xlib: &Xlib, display: *mut _XDisplay, key_code: KeyCode) -> Option<char> {
    let key_sym = unsafe { (xlib.XKeycodeToKeysym)(display, code_for(key_code)? as _, 0) };
    if key_sym == 0 {
        return None;
    }
    let code_point = keysym_to_utf32(key_sym as _);
    if code_point == 0 {
        return None;
    }
...
fn keysym_to_utf32(keysym: u64) -> u32 {
    ...
    const KP_ENTER: u64 = 0xff8d;
    const KP_EQUAL: u64 = 0xffbd;
    const KP_MULTIPLY: u64 = 0xffaa;
    const SPACE: u64 = 0x0020;
    const BACKSPACE: u64 = 0xff08;
    const CLEAR: u64 = 0xff0b;
    const RETURN: u64 = 0xff0d;
    const ESCAPE: u64 = 0xff1b;
    ...
    if matches!(
        ...
        BACKSPACE..=CLEAR |
        KP_MULTIPLY..=KP_9 |
        RETURN | ESCAPE | DELETE | KP_TAB | KP_ENTER | KP_EQUAL
    ) {
        return (keysym & 0x7f) as u32;
    }
